...
{
	out_stream <<  " x = " << obj.x << endl;
	out_stream <<  " y = " << obj.y << endl;
	return out_stream;
}
...
{
	private:
	string label; // Label for the Shape
...
// class Line : private Shape
{
	...
	public:
		...
		void display()
		{
			...
			cout << "label = " << get_label() << endl; // maps to base class
		}		
...
// Note : typically, for this program we should have : shape.cpp, shape.h, line.cpp, line.h, point.cpp, point.h
int main()
{
	Line l1("line1", 10,20,100,200);
	
	l1.set_label("newline1"); // set_label is defined in Shape class.
