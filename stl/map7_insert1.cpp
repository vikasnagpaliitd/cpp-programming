...
int main()
{
	...
	cout << "insert pair(abc,21). Return: ret.first->first = " << ret.first->first << ", ret.first->second = " << ret.first->second << ", ret.second = " << ret.second << endl;
	...
	// Overload 3: Writing very simple code
	{
		...
		auto [it, status] = m1.insert({"abe", 23}); // Overload 3
		cout << "insert pair(abe,23). Return: it->first = " << it->first << ", it->second = " << it->second << ", status = " << status << endl;
		print("m1", m1); // Note: we are getting sorted order listing
	}

	// What happens if element is missing? status becomes false, and it refers to existing element
